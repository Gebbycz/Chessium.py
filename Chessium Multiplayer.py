...
def evaluate_board(b):
    if b.is_checkmate():
        if b.turn == chess.WHITE: return -99999
        else: return 99999
    ...
    for square in chess.SQUARES:
        ...
        if piece:
            ...
            if piece.piece_type == chess.PAWN: val += PAWN_TABLE[idx]
            elif piece.piece_type == chess.KNIGHT: val += KNIGHT_TABLE[idx]
            elif piece.piece_type == chess.BISHOP: val += BISHOP_TABLE[idx]
            elif piece.piece_type == chess.ROOK: val += ROOK_TABLE[idx]
            elif piece.piece_type == chess.QUEEN: val += QUEEN_TABLE[idx]
...
def score_move(b, move):
    score = 0
    if b.gives_check(move): score += 50
...
def alpha_beta(b, depth, alpha, beta, maximizing_player):
    if b.is_game_over(): return evaluate_board(b), None
    if depth == 0: return quiescence_search(b, alpha, beta, maximizing_player), None
...
def play_move_sound(b, move):
    if b.gives_check(move): sound_check.play()
    elif b.is_capture(move): sound_capture.play()
...
def get_move_quality(b, move, max_depth):
    ...
    if move == best_move or diff <= 5: return ("Best Move", "⭐", (255, 215, 0))
    elif diff <= 40: return ("Excellent", "✨", (30, 200, 80))
    elif diff <= 120: return ("Good", "👍", (150, 220, 50))
    elif diff <= 300: return ("Inaccuracy", "?!", (240, 180, 20))
    elif diff <= 600: return ("Mistake", "?", (230, 110, 30))
    else: return ("Blunder", "??", (220, 40, 40))
...
def draw_evaluation_bar(curr_eval):
    ...
    if abs(curr_eval) > 50000: eval_text = "M"
    ...
    while selecting:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if local_rect.collidepoint(pos): return "local"
                if p2p_rect.collidepoint(pos): return "p2p"
...
def connect_multiplayer():
    ...
    while selecting:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
                if host_rect.collidepoint(pos): role = "host"; selecting = False
                elif join_rect.collidepoint(pos): role = "join"; selecting = False
                elif queue_rect.collidepoint(pos): role = "queue"; selecting = False
                elif back_rect.collidepoint(pos): return "BACK_TO_MENU"
...
def main():
    ...
    while True:
        ...
        if mode == "p2p":
            ...
            if game == "BACK_TO_MENU" or not isinstance(game, ChessGame):
                continue
        else:
        ...
        while session_active:
            ...
            if game.game_active and board.is_game_over():
                ...
                result = board.result()
                if result == "1-0": outcome = "win" if player == chess.WHITE else "loss"
                elif result == "0-1": outcome = "win" if player == chess.BLACK else "loss"
                else: outcome = "draw"
            ...
            if game_is_over:
                ...
                if board.is_checkmate(): res_str = "Checkmate! Game Over"
                            ...
                            if piece and piece.color == board.turn:
                                ...
                                if move in board.legal_moves:
                                    ...
                                    if mode == "p2p" and game.conn:
                                        try:
                                            game.conn.sendall(json.dumps({"type": "MOVE", "move": move.uci()}).encode('utf-8'))
                                        except: pass
                                        game.my_turn = False
                                    selected_sq = None
